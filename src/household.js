...
export async function deleteHouseholdContribution(id){const r=await supabase.from('household_contributions').delete().eq('id',id);if(r.error)throw r.error}
export async function deleteHousehold(id){const r=await supabase.from('households').delete().eq('id',id);if(r.error)throw r.error}
